//Source code taken from https://github.com/gadgetron/gadgetron/blob/master/gadgets/dicom/dicom_ismrmrd_utility.cpp
//Hyperthermia edits found at +PROTEUS (ctrl+f to find indexes)
//Details: Keeps the pixel data in float precision. Originally pixels were casted to uint16.
//         Include additional fields necessary for Proteus
//         Removed unused attribs overload of write_ismrmd_image_into_dicom()
//Edited by: Eric Song
...
namespace Gadgetron
{
    ...
    void fill_dicom_image_from_ismrmrd_header(ISMRMRD::IsmrmrdHeader& h, DcmFileFormat& dcmFile)
    {
        try
        {
            ...
            write_dcm_string(dataset, key, "32");
            // Bits Stored
            key.set(0x0028, 0x0101);
            write_dcm_string(dataset, key, "32");
            // High Bit
            key.set(0x0028, 0x0102);
            write_dcm_string(dataset, key, "31");
    ...
    void write_ismrmd_image_into_dicom(ISMRMRD::ImageHeader& m1, hoNDArray<T>& m2, std::string& seriesIUID, DcmFileFormat& dcmFile)
    {
        try
        {
            //+PROTEUS: Keep pixel data in float, instead of casting to uint16
            hoNDArray< Float32 > data;
            boost::shared_ptr< std::vector<size_t> > dims = m2.get_dimensions();
            data.create(dims.get());

            T *src = m2.get_data_ptr();
            Float32 *dst = data.get_data_ptr();
            ...
            for (unsigned long i = 0; i < m2.get_number_of_elements(); i++)
            {
                ...
                dst[i] = static_cast<Float32>(pix_val); //+PROTEUS: Reduntant casting, but kept for consistency with original
            }
            T mean_pix_val = (T)((sum_pix_val * 4) / (T)data.get_number_of_elements());

            /* update the image data_type. */
            //+PROTEUS: data type is kept as float instead of casting to uint16
            m1.data_type = ISMRMRD::ISMRMRD_FLOAT;
            ...
            // +PROTEUS: MAG vs PH depending on image type
            key.set(0x0008, 0x0008);
            switch (m1.image_type) {
                case ISMRMRD::ISMRMRD_IMTYPE_MAGNITUDE :
                    write_dcm_string(dataset, key, "ORIGINAL\\PRIMARY\\MAG");;
                    break;
                case ISMRMRD::ISMRMRD_IMTYPE_PHASE :
                    write_dcm_string(dataset, key, "ORIGINAL\\PRIMARY\\PH");;
                    break;
            }

            //+PROTEUS Insert acquisition time
            key.set(0x0008, 0x0032);
            ACE_OS::snprintf(buf, BUFSIZE, "%d", m1.acquisition_time_stamp);
            write_dcm_string(dataset, key, buf);
            ...
            //+PROTEUS Insert acquistion number
            key.set(0x0020, 0x0012);
            ACE_OS::snprintf(buf, BUFSIZE, "%d", m1.repetition);
            write_dcm_string(dataset, key, buf);
            ...
            //+PROTEUS Insert slice number into IntervalNumber
            key.set(0x0020, 0x0016);
            ACE_OS::snprintf(buf, BUFSIZE, "%d", m1.slice);
            write_dcm_string(dataset, key, buf);
            ...
            key.set(0x7fe0, 0x0008); //+PROTEUS: Key for pixel data now refers to FloatPixelData.

            //+PROTEUS: insert float32 array instead of uint16.
            status = dataset->putAndInsertFloat32Array(key, (Float32*) data.get_data_ptr(), (unsigned long)data.get_number_of_elements());
            if (!status.good()) {
            ...
            // Trigger Time
            uint32_t ticks = m1.physiology_time_stamp[0]; // +PROTEUS: Moved from attribs
    ...
    template EXPORTGADGETSPROTEUS void write_ismrmd_image_into_dicom(ISMRMRD::ImageHeader& m1, hoNDArray<short>& m2, std::string& seriesIUID, DcmFileFormat& dcmFile);
    template EXPORTGADGETSPROTEUS void write_ismrmd_image_into_dicom(ISMRMRD::ImageHeader& m1, hoNDArray<unsigned short>& m2, std::string& seriesIUID, DcmFileFormat& dcmFile);
    template EXPORTGADGETSPROTEUS void write_ismrmd_image_into_dicom(ISMRMRD::ImageHeader& m1, hoNDArray<int>& m2, std::string& seriesIUID, DcmFileFormat& dcmFile);
    template EXPORTGADGETSPROTEUS void write_ismrmd_image_into_dicom(ISMRMRD::ImageHeader& m1, hoNDArray<unsigned int>& m2, std::string& seriesIUID, DcmFileFormat& dcmFile);
    template EXPORTGADGETSPROTEUS void write_ismrmd_image_into_dicom(ISMRMRD::ImageHeader& m1, hoNDArray<float>& m2, std::string& seriesIUID, DcmFileFormat& dcmFile);
    template EXPORTGADGETSPROTEUS void write_ismrmd_image_into_dicom(ISMRMRD::ImageHeader& m1, hoNDArray<double>& m2, std::string& seriesIUID, DcmFileFormat& dcmFile);
}
